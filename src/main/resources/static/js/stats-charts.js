// /admin/stats(전체 사용자 합산)와 /my/stats(개인 통계)가 화면 구성이 완전히 같아서
// (방문국가 도넛/연도별 막대/대륙별 막대 3종 + 테마 연동) 데이터를 가져올 URL만 다르게
// 받는 공용 렌더러로 뺐다 — admin-stats.js/my-stats.js는 이 함수를 부르기만 하는 얇은
// 래퍼로 남는다.
window.GlobelogStatsCharts = (function () {
    ...
    function init(dataUrl) {
        ...
        // 테마 토글(admin.js/theme.js가 <html data-theme>을 바꿈) 시 차트 색상을 다시 그린다.
